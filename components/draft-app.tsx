...
import type { DraftCandidateView, LiveDraftViewModel, PlayerView } from '@/lib/live/types';
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch('/api/discover', {
        ...
        body: JSON.stringify({ username, draftId }),
      });
      ...
      <div className="mx-auto grid max-w-6xl items-center gap-10 lg:grid-cols-[1.05fr_.95fr]">
        ...
        <section className="rounded-[28px] border border-black/10 bg-white p-6 shadow-[0_24px_70px_rgb(23_47_38/12%)] sm:p-8" aria-labelledby="connect-title">
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            <label className="block">
              <span className="mb-2 block text-xs font-bold uppercase tracking-[0.1em] text-[#52655c]">Draft ID or URL <em className="font-normal normal-case text-[#849087]">optional</em></span>
              <input value={draftId} onChange={(event) => setDraftId(event.target.value)} placeholder="Paste it for the fastest match" className="h-12 w-full rounded-xl border border-black/15 bg-[#faf9f6] px-4 outline-none transition focus:border-[#2f6f55] focus:ring-4 focus:ring-[#73e2a7]/20" />
            </label>
          ...
          {discovery && discovery.candidates.length === 0 && (
            <p className="mt-4 rounded-xl bg-[#f3f1eb] p-4 text-sm">No current drafts were found. Paste a direct draft URL or try the sample room.</p>
          )}
        ...
        <section className={`mb-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border px-4 py-3 ${view.turn.state === 'on-clock' ? 'border-[#b1c948] bg-[#e8ff71]' : 'border-black/10 bg-white'}`} role={view.turn.state === 'on-clock' ? 'alert' : 'status'}>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="flex items-center gap-2 rounded-full bg-white/65 px-3 py-2 text-xs font-bold">
              <span className={`h-2 w-2 rounded-full ${error ? 'bg-[#df8c32]' : 'bg-[#42a874]'}`} />
              {error ? 'Update delayed' : 'Live'}
            </span>
          </div>
        </section>

        {error && <p role="alert" className="mb-4 rounded-xl bg-[#fff2dc] px-4 py-3 text-sm font-medium text-[#81531e]">{error} Showing the last confirmed board.</p>}
          ...
          <section className={`order-1 overflow-hidden rounded-[28px] bg-[#18372b] text-white shadow-[0_22px_60px_rgb(19_47_37/18%)] lg:order-2 ${view.turn.state === 'on-clock' ? 'ring-4 ring-[#e8ff71]/70' : ''}`} aria-labelledby="recommendation-title">
            ...
            {recommendation ? (
              <div className="px-6 py-7 sm:px-8 sm:py-9">
                ...
                <p className="mt-5 text-[11px] text-white/42">Fast deterministic model · {recommendation.modelVersion} · availability must be verified in Sleeper</p>
              </div>
            ) : (
...
export default function DraftApp() {
  ...
  const refresh = useCallback(async (nextConnection: Connection) => {
    try {
      ...
      const response = await fetch(`/api/draft?${params}`, { cache: 'no-store' });
  ...
  const body = useMemo(() => {
    ...
    return <ConnectScreen onPreview={() => setIsDemo(true)} onConnected={(next) => { setConnection(next); setLoading(true); }} />;
  }, [connection, error, isDemo, loading, view]);
